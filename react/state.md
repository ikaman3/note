...
1. 컴포넌트의 시각적 state를 확인
2. 무엇인 state 변화를 트리거하는지 파악
3. `useState`를 사용해서 메모리의 state를 표현
4. 불필요한 state 변수 제거
5. state 설정을 위해 이벤트 핸들러 연결

#### 첫 번째: 컴포넌트의 시각적 state 확인

> 사용자가 볼 수 있는 UI의 모든 state를 시각화한다.  
>  
> - Empty: form은 비활성화된 '제출' 버튼을 가짐
> - Typing: form은 활성화된 '제출' 버튼을 가짐
> - Submitting: form은 비활성화되고 스피너가 보임
> - Success: form 대신 '감사합니다' 등의 메시지가 보임
> - Error: Typing state와 동일하지만 오류 메시지가 보임

로직을 추가하기 전에 state를 mockup 하거나 모형을 만들어서 테스트할 수 있다.

```javascript
export default function Form({
    // 'submitting', 'error', 'success'로 변경해보며 테스트
    status = 'success'
}) {
    if (status === 'success') {
...
컴포넌트가 많은 state를 가졌다면 한 페이지에서 모두 보여주는 것도 좋다.  

```javascript
// 위의 코드를 <Form> 으로 import
...
> - 이러한 페이지를 "Living styleguides(살아있는 스타일가이드)" 또는 Storybook(스토리북)이라고 부른다.  