...
어떤 페이지(`/some-route`)는 데이터가 필요로하고 이 라우트에 요청이 들어오면 사전 렌더링된 페이지를 반환한다.  
표준 React는 빈 HTML 파일과 모든 JavaScript 코드가 표시될 것이다. 일반 DOM 구조 외에 필요한 데이터가  
서버로부터 로딩될 때에는 시간이 걸린다.
